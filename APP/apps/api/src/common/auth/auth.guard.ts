...
import { sessionIsExpired } from "./session-expiry";

export const IS_PUBLIC = "isPublic";
/** Routes publiques (inscription, connexion, OTP) — tout le reste est authentifié par défaut. */
export const Public = () => SetMetadata(IS_PUBLIC, true);

/* ENF-07 — la règle vit dans `session-expiry.ts` depuis le 06/09/2026 (chantier 53) : la garde
   qui REFUSE et la liste qui MONTRE doivent dire la même chose. Ré-export pour ne rien casser. */
export { WEB_IDLE_SECONDS } from "./session-expiry";
...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    if (sessionIsExpired(session.client, session.lastActiveAt.getTime(), Date.now(), pm20)) {
