...
| 8 | **Tests d'intégration API à l'arrêt** — il manque une **branche Neon de test** et son `TEST_DATABASE_URL`. Tout le reste est prêt : le garde-fou `test/garde-base-de-test.ts` refuse de démarrer sans elle (et refuse aussi qu'on y recopie l'URL de production — le geste exact qui a effacé la base le 23/08), et `.env.example` porte la ligne à remplir.
