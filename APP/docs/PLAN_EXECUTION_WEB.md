...
| 11 | **La coquille applicative (B1) n'a aucun test de rendu** | Barre latérale, en-tête, menu utilisateur, navigation filtrée par rôle : rien ne verrouille son comportement. Deux défauts y sont déjà passés — le `className` transformé en chaîne par `Slot`, et l'absence d'anneau de focus. Les autres écrans construits sont couverts depuis le 24/08 (A1, A2, A3, A4, B2, B3, C1). |
