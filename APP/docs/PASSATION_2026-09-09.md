...
|---|---|---|
| API | `npm run test:unit` **uniquement** | **687 ✓** (41 suites) |
...
## 3. Ce qui a été fait — chantiers 58 à 118
...
| 118 | *(à pousser)* | **Le prix se fige à la demande.** Le montant était relu dans l'offre au moment de payer : un tarif modifié entre-temps faisait payer au patient un prix qu'il n'avait jamais vu. Prix, libellé et durée figés sur la poignée. ⚠️ **Migration à appliquer au déploiement.** |
...
| 113 | *(à pousser)* | **Le carnet de numéros Mobile Money — le serveur.** L'ordre de débit partait vers le numéro de CONNEXION quel que soit l'opérateur choisi. Table `MomoNumber` (un numéro par opérateur), routes `/v1/accounts/me/momo`, paiement qui s'en sert, préfixe qui prévient sans interdire. ⚠️ **Migration à appliquer au déploiement.** |
| 112 | *(à pousser)* | **L'export du Carnet retiré du téléphone** (décision du porteur). Le bouton disparaît ; la route serveur et l'empreinte sha256 restent — *ce qui disparaît est le bouton, pas le droit.* Le code de remise familial est conservé. |
...
| 98 | *(à pousser)* | **Le bandeau de la discussion dit enfin avec QUI l'on parle** : visage (photo ou initiales), prénom, état de la conversation, minuteur et retard. ⚠️ **Le statut n'existait pas** — l'application du patient n'envoyait aucun battement de présence ; elle en envoie un, et le serveur sait dire le quatrième état, « on ne sait pas ». 📌 Le **retard** devient visible par le soignant (asymétrie des chantiers 93-94, tranchée). ⚠️ Un test est tombé en déplaçant le minuteur : j'avais réduit « Horloge serveur » à une infobulle. Plus le compteur vocal qui monte et le zéro barré. |
...
| 89 | *(à pousser)* | **Le compte-rendu tardif : accepté, sans crédit.** Trouvé par le porteur, bloqué au dépôt. Le serveur refusait passé 24 h, **définitivement et sans recours** — le Carnet du patient perdait le compte-rendu d'une consultation qui avait eu lieu. Le dépôt passe désormais, `capture()` non : la sanction est entière, le dossier est sauvé. ⚠️ **L'écran promettait une chose et son contraire**, chacune avec son test : *un test de promesse garde la formulation, pas la vérité*. Ouvre la **dette n°29**. |
