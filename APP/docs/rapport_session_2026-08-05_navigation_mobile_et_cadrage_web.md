...
| 2 | 🔴 | **Verrouillage définitif possible.** `ForgotPasswordPage` ne récupère **que** par TOTP (règle assumée : jamais de SMS ni d'email pour un compte web). Mais l'inscription **n'impose pas** le passage par `TotpSetupPage`. Sans TOTP → aucune voie de récupération |
